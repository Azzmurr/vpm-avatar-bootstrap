Shader "Hidden/Locked/.poiyomi/Poiyomi Pro/3bf16a5128a0b1347b89c53a187c917a"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.3.22</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				lightMap.x *= lerp(1.0, shadowAttenuationNoStrength, 0.0);
				...
				if (1.0)
				{
					...
					lns.y = poiEdgeNonLinearNoSaturate(lightMap.y, 0.5, 0.8 * blurMap.g);
					lns.z = poiEdgeNonLinearNoSaturate(lightMap.z, 0.25, 0.6 * blurMap.b);
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r, 1.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.8 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.6 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.5, 1.0 * blurMap.r, 1.0);
				}
				#if defined(PROP_SHADOWBORDERMASK) || !defined(OPTIMIZER_ENABLED)
				...
				if (poiFragData.toggleVertexLights)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						if (lightingMode == 1)
						{
							float2 ToonAddGradient = float2(0.0, 0.5);
							if (ToonAddGradient.x == ToonAddGradient.y) ToonAddGradient.y += 0.0001;
							vertexLighting = max(vertexLighting, lerp(poiLight.vColor[index], poiLight.vColor[index] * 0.1, smoothstep(ToonAddGradient.x, ToonAddGradient.y, 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow);
						}
					}
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (1.0)
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						poiLight.vColor[index] = 1.0 ? MaxLuminance(unity_LightColor[index].rgb * poiLight.vAttenuation[index], 3.0) : unity_LightColor[index].rgb * poiLight.vAttenuation[index];
				...
				if (1.0)
				{
					poiLight.directColor = min(poiLight.directColor, 3.0);
					poiLight.indirectColor = min(poiLight.indirectColor, 3.0);
				}
				if (0.0)
				{
				...
				poiLight.directColor = 1.0 ? MaxLuminance(_LightColor0.rgb * poiLight.attenuation, 3.0) : _LightColor0.rgb * poiLight.attenuation;
				#if defined(POINT_COOKIE) || defined(DIRECTIONAL_COOKIE)
				poiLight.indirectColor = 0;
				#else
				poiLight.indirectColor = lerp(0, poiLight.directColor, 0.1);
				poiLight.indirectColor = 1.0 ? MaxLuminance(poiLight.indirectColor, 3.0) : poiLight.indirectColor;
				#endif
				poiLight.directColor = lerp(poiLight.directColor, dot(poiLight.directColor, float3(0.299, 0.587, 0.114)), 0.1);
				poiLight.indirectColor = lerp(poiLight.indirectColor, dot(poiLight.indirectColor, float3(0.299, 0.587, 0.114)), 0.1);
				poiLight.halfDir = normalize(poiLight.direction + poiCam.viewDir);
